...
def make_blocks(
    ...
    for stage_idx, (planes, num_blocks, db) in enumerate(zip(channels, block_repeats, drop_blocks(drop_block_rate))):
        ...
        block_kwargs = dict(reduce_first=reduce_first,
                            dilation=dilation, drop_block=db, **kwargs)
        blocks = []
...
class CifarResNet(nn.Module):
    ...
    def forward(self, x, labels=None, args=None):
        ...
        def power_grad(grad):
            nonlocal labels, args
            q = torch.ones(1, dtype=grad.dtype, device=grad.device) * \
                F.one_hot(labels, num_classes=args.num_classes)
            q = (1.0 - args.smoothing) * q + args.smoothing / args.num_classes
            g = grad / grad.shape[0]
            p = g + q

            p = p**(args.modify_softmax)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, in_chans=3,
        ...
        if deep_stem:
            ...
            self.conv1 = nn.Sequential(*[
                nn.Conv2d(in_chans, stem_chs[0], 3,
                          stride=2, padding=1, bias=False),
                norm_layer(stem_chs[0]),
                act_layer(inplace=True),
                nn.Conv2d(stem_chs[0], stem_chs[1], 3,
        ...
        if replace_stem_pool:
            self.maxpool = nn.Sequential(*filter(None, [
                nn.Conv2d(inplanes, inplanes, 3,
                          stride=1 if aa_layer else 2, padding=1, bias=False),
    ...
    def forward(self, x, labels=None, args=None):
        ...
        def softmax_smoothing_grad(grad):
            nonlocal labels
            d, C = 0.1, 1000
            q = torch.ones(1, dtype=grad.dtype, device=grad.device) * \
        ...
        def print_grad_stats(grad):
            g = grad / grad.shape[0]
            self.gstat_str = 'GStat: {:5.2f},{:5.2f}  '.format(g.min(), g.max())

        if self.training:
            if args.modify_softmax:
                x.register_hook(power_grad)
            if args.grad_stats:
                x.register_hook(print_grad_stats)

        return x, self.gstat_str
...
@register_model
def resnet18_wobn(pretrained=False, **kwargs):
    """Constructs a ResNet-18 Non-BN model.
    """
    model_args = dict(block=BasicBlock, layers=[
                      2, 2, 2, 2], norm_layer=nn.Identity, **kwargs)
    return _create_resnet('resnet18_wobn', pretrained, **model_args)
...
def resnet20(pretrained=False, **kwargs):
    return CifarResNet(CifarBasicBlock, [3, 3, 3], kwargs['num_classes'])


@register_model
def resnet32(pretrained=False, **kwargs):
    return CifarResNet(CifarBasicBlock, [5, 5, 5], kwargs['num_classes'])


@register_model
def resnet44(pretrained=False, **kwargs):
    return CifarResNet(CifarBasicBlock, [7, 7, 7], kwargs['num_classes'])


@register_model
def resnet56(pretrained=False, **kwargs):
    return CifarResNet(CifarBasicBlock, [9, 9, 9], kwargs['num_classes'])


@register_model
def resnet110(pretrained=False, **kwargs):
    return CifarResNet(CifarBasicBlock, [18, 18, 18], kwargs['num_classes'])


@register_model
def resnet1202(pretrained=False, **kwargs):
    return CifarResNet(CifarBasicBlock, [200, 200, 200], kwargs['num_classes'])
...
def resnet26d(pretrained=False, **kwargs):
    ...
    model_args = dict(block=Bottleneck, layers=[
                      2, 2, 2, 2], stem_width=32, stem_type='deep', avg_down=True, **kwargs)
...
def resnet101d(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(block=Bottleneck, layers=[
                      3, 4, 23, 3], stem_width=32, stem_type='deep', avg_down=True, **kwargs)
...
def resnext50_32x4d(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(block=Bottleneck, layers=[
                      3, 4, 6, 3], cardinality=32, base_width=4, **kwargs)
...
def resnext101_32x4d(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(block=Bottleneck, layers=[
                      3, 4, 23, 3], cardinality=32, base_width=4, **kwargs)
...
def resnext101_32x8d(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(block=Bottleneck, layers=[
                      3, 4, 23, 3], cardinality=32, base_width=8, **kwargs)
...
def resnext101_64x4d(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(block=Bottleneck, layers=[
                      3, 4, 23, 3], cardinality=64, base_width=4, **kwargs)
...
def ig_resnext101_32x16d(pretrained=True, **kwargs):
    ...
    """
    model_args = dict(block=Bottleneck, layers=[
                      3, 4, 23, 3], cardinality=32, base_width=16, **kwargs)
...
def ig_resnext101_32x32d(pretrained=True, **kwargs):
    ...
    """
    model_args = dict(block=Bottleneck, layers=[
                      3, 4, 23, 3], cardinality=32, base_width=32, **kwargs)
...
def ig_resnext101_32x48d(pretrained=True, **kwargs):
    ...
    """
    model_args = dict(block=Bottleneck, layers=[
                      3, 4, 23, 3], cardinality=32, base_width=48, **kwargs)
...
def resnetblur18(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(block=BasicBlock, layers=[
                      2, 2, 2, 2], aa_layer=BlurPool2d, **kwargs)
...
def resnetblur50(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(block=Bottleneck, layers=[
                      3, 4, 6, 3], aa_layer=BlurPool2d, **kwargs)
...
def seresnet18(pretrained=False, **kwargs):
    model_args = dict(block=BasicBlock, layers=[
                      2, 2, 2, 2], block_args=dict(attn_layer='se'), **kwargs)
...
def seresnet34(pretrained=False, **kwargs):
    model_args = dict(block=BasicBlock, layers=[
                      3, 4, 6, 3], block_args=dict(attn_layer='se'), **kwargs)
...
def seresnet101(pretrained=False, **kwargs):
    model_args = dict(block=Bottleneck, layers=[
                      3, 4, 23, 3], block_args=dict(attn_layer='se'), **kwargs)
...
def seresnet152(pretrained=False, **kwargs):
    model_args = dict(block=Bottleneck, layers=[
                      3, 8, 36, 3], block_args=dict(attn_layer='se'), **kwargs)
